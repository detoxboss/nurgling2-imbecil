...
| [`fork-sync-guide.md`](fork-sync-guide.md) | current | Syncing this fork against the `upstream` remote (`aleksandrsvoboda/nurgling2`) |
| [`nurgling-modified-files.md`](nurgling-modified-files.md) | current | File-level list of every `haven`-package file carrying nurgling changes, and per-file conflict strategy |
